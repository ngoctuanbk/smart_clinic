...
var Layout = function () {
    ...
    var activeSideBar = function(){
        var menu = $('.page-sidebar-menu');
        menu.find('li.nav-item').each((i) =>{
            let elCurrent = menu.find('li.nav-item:eq('+ i + ')');
            let atv = elCurrent.hasClass('open');
            if(atv){
                elCurrent.parents('li').addClass('active').addClass('open');
                menu.find('li.nav-item.active.open a span.arrow').addClass('open');
            }
        })
    }
    return {
